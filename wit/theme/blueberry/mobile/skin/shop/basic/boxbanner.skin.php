...
for ($i=0; $row=sql_fetch_array($result); $i++)
{
    ...
    if ($i==0) echo '<ul class="side_bn">'.PHP_EOL;
    //print_r2($row);
    // 테두리 있는지
    ...
    if (file_exists($bimg))
    {
        ...
        if($banner)
            echo '</a>'.PHP_EOL;
        echo '<a href="'.G5_SHOP_URL.'/bannerhit.php?bn_id='.$row['bn_id'].'&amp;url='.urlencode($row['bn_url']).'"'.$bn_new_win.' class="bn-txt"><span>'.$row['bn_alt'].'</span></a>'.PHP_EOL;
        echo '</li>'.PHP_EOL;
    }
}
