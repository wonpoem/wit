...
<aside id="sbsk">
    ...
    for ($i=0; $row=sql_fetch_array($result); $i++)
    {
        ...
        echo '<li><div class="cart_wr">'.PHP_EOL;
        $it_name = get_text($row['it_name']);
        $it_img = get_it_image($row['it_id'], 80, 100, true);
        ...
        for($k=0; $row2 = sql_fetch_array($res2); $k++) {
            echo '<span class="cart_op">'.get_text($row2['ct_option']).'<span class="cart_qty"> +'.number_format($row2['ct_qty']).'</span></span>'.PHP_EOL;
        ...
        echo '</span>'.PHP_EOL;
        echo '<button class="cart_del" type="button" data-it_id="'.$row['it_id'].'"><span class="sound_only">삭제</span><i class="fa fa-times" aria-hidden="true"></i></button>'.PHP_EOL;
        echo '</div></li>'.PHP_EOL;
    }
    ...
    <div class="cart_btn">
        ...
        <a href="<?php echo G5_SHOP_URL; ?>/cart.php">장바구니 가기</a>
    </div>
    </form>
